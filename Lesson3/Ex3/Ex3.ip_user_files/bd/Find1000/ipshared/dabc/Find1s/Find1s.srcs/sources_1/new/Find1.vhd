...
entity Find1 is 
    ...
    btnC    : in std_logic; 
    inp_ut     : in  STD_LOGIC_VECTOR (number_of_bits-1 downto 0); 
    led     : out  STD_LOGIC_VECTOR (10 downto 0) 
    ); 
end Find1; 
 
architecture Behavioral of Find1 is 
...
process (C_S, inp_ut, index, n_o_ones, Res)  -- processo combinat?rio   
begin 
   N_S     <= C_S; 
   next_index     <= index; 
...
case C_S is 
  when initial_state => next_index <= index + 1; N_S <= initial_state; 
         next_n_o_ones <= n_o_ones + conv_integer(inp_ut(index)); 
...
led <= conv_std_logic_vector(Res, 11);   -- resultado 
