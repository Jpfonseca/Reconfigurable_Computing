...
void putnumVGA(unsigned int number,unsigned int b);
int main()
{
    init_platform();
		...
		print_2vga(s6,2010);
		putnumVGA(value_addr,2120);
		print_2vga(s8,2510);
		putnumVGA(value_hamm,2620);
...
void putnumVGA(unsigned int number,unsigned int b)
{
	...
	else
	{
		...
		for(unsigned int order = 100000000; order > 0; order /= 10)
		{
			if(number >= order)
			{
				int digit = (temp - (temp % order)) / order;

				print_2vga(48 + digit,b);
